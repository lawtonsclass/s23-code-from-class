...
password2:     file format elf64-x86-64
...
0000000000001149 <main>:
    ...
    1158:	48 89 75 f0          	mov    QWORD PTR [rbp-0x10],rsi   # argv
    115c:	48 8b 45 f0          	mov    rax,QWORD PTR [rbp-0x10]   # rax = argv
    1160:	48 83 c0 08          	add    rax,0x8                    # rax += 8 (now it holds &argv[1])
    1164:	48 8b 00             	mov    rax,QWORD PTR [rax]        # rax = *rax (now it holds argv[1])
    1167:	48 83 c0 03          	add    rax,0x3                    # rax += 3 (addr of char 4 in argv[1])
    116b:	0f b6 00             	movzx  eax,BYTE PTR [rax]         # eax = *rax (eax = 4th char in argv[1])
    116e:	3c 43                	cmp    al,0x43                    # compare the char to 0x43 ('C')
    1170:	75 11                	jne    1183 <main+0x3a>    # if not equal, print access denied
    1172:	48 8d 05 8b 0e 00 00 	lea    rax,[rip+0xe8b]        # 2004 <_IO_stdin_used+0x4>
    1179:	48 89 c7             	mov    rdi,rax
    117c:	e8 cf fe ff ff       	call   1050 <puts@plt>       # prints access granted
    1181:	eb 0f                	jmp    1192 <main+0x49>     # go to end of program (so you don't also print access denied)
    1183:	48 8d 05 89 0e 00 00 	lea    rax,[rip+0xe89]        # 2013 <_IO_stdin_used+0x13>
    118a:	48 89 c7             	mov    rdi,rax
    118d:	e8 be fe ff ff       	call   1050 <puts@plt>
    1192:	b8 00 00 00 00       	mov    eax,0x0       # return 0
    1197:	c9                   	leave  
    1198:	c3                   	ret    
