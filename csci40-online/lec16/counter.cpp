...
class Counter {
  ...
  private:
    int count; // count inside the Counter
...
// If I skip a member variable in the initializer list, it gets
// default constructed
...
int main() {
  ...
  cout << (c < sum) << endl; // c.operator<(sum) / operator<(c, sum)
